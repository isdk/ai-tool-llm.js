...
export const TOKENIZER_NAMES_MAP: any = {
...
export async function getTokenizer(modelId: string) {
...
export async function encodeTokens(text: string, modelId: string = 'gpt-4') {
  if (TOKENIZER_NAMES_MAP[modelId]) { modelId = TOKENIZER_NAMES_MAP[modelId] }
  const tokenizer = await getTokenizer(modelId)
  return tokenizer.encode(text)
}

export async function countTokens(text: string, modelId: string = 'gpt-4') {
  const encoded = await encodeTokens(text, modelId)
  console.log('🚀 ~ countTokens ~ encoded:', encoded)
